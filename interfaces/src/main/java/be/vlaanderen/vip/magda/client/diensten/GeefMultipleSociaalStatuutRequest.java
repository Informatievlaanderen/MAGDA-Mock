...
public class GeefMultipleSociaalStatuutRequest extends PersonMagdaRequest {
    ...
    public static class Builder extends PersonMagdaRequest.Builder<Builder> {
        ...
        public GeefMultipleSociaalStatuutRequest build() {
            ...
            if(socialStatutes == null) { throw new IllegalStateException("socialStatutes must be given"); }
            if(socialStatutes.size() < 1) { throw new IllegalStateException("socialStatutes must contain at least 1 entry"); }
            if(socialStatutes.size() > 8) { throw new IllegalStateException("socialStatutes cannot contain more than 8 entries"); }
    ...
    public GeefMultipleSociaalStatuutRequest(
            ...
            @Nullable Set<SociaalStatuutRequestCriteria> socialStatutes) {
    ...
    protected void fillIn(MagdaDocument request, MagdaRegistrationInfo magdaRegistrationInfo) {
        ...
        socialStatutes.forEach(x -> {
            writeSocialeStatuteCriteria(request, x);
        });
    }

    private void writeSocialeStatuteCriteria(MagdaDocument request, SociaalStatuutRequestCriteria socialStatute)
    {
        /*
            <SociaalStatuut>
                <Naam>Naam</Naam>
