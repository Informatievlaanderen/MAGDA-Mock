...
class SigningTest {
    ...
    void signingMetX509Certificaat() {
        try (var resource = this.getClass().getResourceAsStream("/certificates/mock_x509_certificaat.pfx")) {
            var signedDocument = signWithKeystore(TYPESTORE_PKCS_12, resource, STORE_PASSWORD, CERTIFICATE_NAME, SUBJECTDN);
            try (var resource2 = this.getClass().getResourceAsStream("/certificates/mock_x509_certificaat.pfx")) {
    ...
    void signingMetJKSStore() {
        try (var resource = Objects.requireNonNull(this.getClass().getResourceAsStream("/certificates/mock keystore.jks"))) {
    ...
    private void assertDocumentIsSigned(MagdaDocument signedDocument, String typeStore, InputStream resource, String storePassword) {
        ...
        var crypto = loadKeystore(typeStore, resource, storePassword);
    ...
    private MagdaDocument signWithKeystore(String typeStore, InputStream resource, String storePaswoord, String certificaatNaam, String subjectDN) throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException, WSSecurityException {
        var crypto = loadKeystore(typeStore, resource, storePaswoord);

        var cryptoType = cryptoTypeSubjectDN(subjectDN);

        try (var request = Objects.requireNonNull(this.getClass().getResourceAsStream("/requests/GeefPersoon.xml"))) {
            return MagdaDocument.fromDocument(signDocument(request, crypto, cryptoType, certificaatNaam, storePaswoord));
        }
    }


    private Document signDocument(InputStream request, Crypto crypto, CryptoType cryptoType, String certificaatNaam, String storePaswoord) throws WSSecurityException {
        ...
        var encP = new WSEncryptionPart(WSConstants.ELEM_BODY, soapNamespace, "");
    ...
    private CryptoType cryptoTypeSubjectDN(String subjectDN) {
        var cryptoType = new CryptoType(CryptoType.TYPE.SUBJECT_DN);
    ...
    private Merlin loadKeystore(String typeStore, InputStream resource, String storePaswoord) throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException, WSSecurityException {
        var store = KeyStore.getInstance(typeStore);
        ...
        var crypto = (Merlin) CryptoFactory.getInstance(properties);
